...
export default function Register() {
  ...
  async function handleSubmit(e) {
    ...
    try { const u = await register(form); navigate(`/${u.role}`); }
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-paper px-4 py-10">
      ...
      <div className="relative w-full max-w-sm animate-fade-in">
        <div className="mb-6 text-center">
          <h1 className="font-display text-2xl font-bold text-ink">Join <span className="text-amber">South Shopping</span></h1>
            ...
            <div className="grid gap-2">
              {ROLES.map(r => (
                <button type="button" key={r.key} onClick={()=>up('role',r.key)}
                  ...
                  {form.role===r.key && <span className="ml-auto text-amber text-sm">✓</span>}
                </button>
              ))}
            </div>
            {[
              ...
              {label:'Password',field:'password',type:'password',ph:'6+ characters',req:true},
            ].map(f=>(
              <div key={f.field}>
        ...
        <p className="mt-4 text-center text-sm text-inkmuted">Already have an account? <Link to="/login" className="font-semibold text-amber hover:opacity-80">Sign in</Link></p>
      </div>
    </div>
  );
}
