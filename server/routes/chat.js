...
router.get('/conversations', authRequired, async (req, res) => {
  try {
    ...
    const convs = result.rows.map(c => ({
      ...
      participant1: { id: c.participant1, name: c.p1_name, role: c.p1_role },
      participant2: { id: c.participant2, name: c.p2_name, role: c.p2_role },
...
router.post('/conversations', authRequired, async (req, res) => {
  try {
    ...
    let result = await pool.query(
      'SELECT * FROM conversations WHERE participant1=$1 AND participant2=$2 AND (order_id=$3 OR ($3::integer IS NULL AND order_id IS NULL))',
      [p1, p2, orderId || null]
    );
    ...
    result = await pool.query(
      ...
      [type || 'support', orderId || null, p1, p2]
    );
...
router.post('/conversations/:id/messages', authRequired, async (req, res) => {
  try {
    ...
    res.status(201).json({ message: { id: m.id, body: m.body, createdAt: m.created_at, sender: { id: req.user.id, name: req.user.name, role: req.user.role } } });
...
router.get('/unread', authRequired, async (req, res) => {
  ...
  } catch (err) { res.status(500).json({ unread: 0 }); }
});
